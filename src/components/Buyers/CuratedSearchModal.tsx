...
import { CuratedSearchSection } from './CuratedSearchSection'

interface CuratedSearchModalProps {
  isOpen: boolean
  onClose: () => void
}

export const CuratedSearchModal = ({ isOpen, onClose }: CuratedSearchModalProps) => {
      ...
      <div className='fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4'>
        <div className='bg-white dark:bg-dark rounded-lg shadow-2xl w-full max-w-5xl max-h-[95vh] overflow-hidden flex flex-col'>
          ...
          <div className='sticky top-0 bg-white dark:bg-dark border-b border-gray-200 dark:border-gray-700 px-4 sm:px-6 py-4 flex items-center justify-between flex-shrink-0'>
            <h2 className='text-lg sm:text-2xl font-bold truncate'>Welcome Home</h2>
            <button
              onClick={onClose}
          ...
          <div className='overflow-y-auto flex-1'>
            <div className='px-4 sm:px-6 py-6'>
              <CuratedSearchSection onSuccess={onClose} />
            </div>
          </div>
        </div>
      </div>
